...
AmpPhaseFDWaveform* CreateAmpPhaseFDWaveform(
    ...
    wf->time = (double*) malloc(sizeof(double) * length);
    if ((wf->freq == NULL) || (wf->amp == NULL) || (wf->phase == NULL) || (wf->time == NULL))
    ...
    memset(wf->time, 0, sizeof(double) * length);
    return wf;
}
...
int init_useful_powers(UsefulPowers *p, double number)
{
    ...
    // hack to overide checks and keep MCMC running
    number = fabs(number);
