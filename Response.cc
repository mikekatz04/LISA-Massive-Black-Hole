...
void setup_noise_free(struct Data *dat, double *params, double Tobs, double dt)
{
           ...
           double f;
           dat->Tobs = Tobs; // atof(argv[1]);
           dat->sqrtTobs = sqrt(dat->Tobs);
           dat->Tstart = 0.0;
           dat->Tend = dat->Tobs;
           dat->dt = dt; // atof(argv[2]);
           dat->Nch = 2;  // only analyze A, E
           dat->N = (int)(dat->Tobs/dat->dt);
           dat->SN = double_matrix(dat->Nch,dat->N/2);
           dat->SM = double_matrix(dat->Nch,dat->N/2);
        ...
        //   FILE *in = fopen("source.dat","r");
        //   for(int i=0; i< NP; i++) fscanf(in,"%lf", &params[i]);
        //   fclose(in);
        ...
        double *AS = double_vector(dat->N);
        double *ES = double_vector(dat->N);
...
HetLikeWrap::HetLikeWrap(double *init_params, double Tobs_, double dt_, bool is_noise_free, int seg_, int rep_, double *premove, int NS)
{
  ...
  max_val = new double[NP];
  min_val = new double[NP];
...
void HetLikeWrap::udpate_heterodyne(double *params)
{
    freehet(het);
...
{
    return log_likelihood_het(dat, het, ll, params, sx[0]);
}

void HetLikeWrap::dealloc()
{
    freehet(het);
    delete het;
...
void ResponseFreq(struct Data *dat, int ll, double *params, double *AS, double *ES)
{
   ...
   // printf("%e %e %e\n", fstart, fstop, fr);
    ...
    // for (i=0; i< nf; i++) 
    // printf("CHECK: %d %e %e %e \n", nf, fstart, fstop, Tobs);
...
void StartStop(int ll, double *params, double Tstart, double Tend, double dt, double *fstart, double *fstop, double *frg)
{
    ...
    *fstart = fmin;
    *fstop = fmax;
    // printf("CHECK 2 %e %e %e %e %e %e %e\n", fmin, fmax, fr, Tseg, Tstart, Tend, dt);
}
...
double FofT(int ll, double Tobs, double *params, double *frg, double dt, double tref)
{
    ...
    // printf("CHECK 4: %e %e %e %e\n", distance, Mc, Mtot, eta);
     if(eta > 0.25)
     {
        dm = 0.0;
     }
