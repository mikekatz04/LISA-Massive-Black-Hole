#ifndef __IMRPHENOMD_INTERNALS_H__
#define __IMRPHENOMD_INTERNALS_H__
...
int IMRPhenomDGenerateFD(
  ...
  if (*htilde) ERROR(PD_EFAULT, "");
    if (fRef_in < 0.0) ERROR(PD_EDOM, "fRef_in must be positive (or 0 for 'ignore')\n");
    if (deltaF <= 0.0) ERROR(PD_EDOM, "deltaF must be positive\n");
    if (m1 <= 0.0) ERROR(PD_EDOM, "m1 must be positive\n");
    if (m2 <= 0.0) ERROR(PD_EDOM, "m2 must be positive\n");
    if (f_min <= 0.0) ERROR(PD_EDOM, "f_min must be positive\n");
  ...
  //if (f_max_prime <= f_min) ERROR(PD_EDOM, "f_max <= f_min\n");
...
std::complex<double> complex_exp(std::complex<double> x)
{
    std::complex<double> out(sin(x.real()), cos(x.imag()));
}
  ...
  *htilde = CreateCOMPLEX16FrequencySeries("htilde: FD waveform", ligotimegps_zero, 0.0, deltaF, n);
  ...
  for (size_t i = ind_min; i < ind_max; i++)
  {
    ...
    else
    {
      ...
      ((*htilde)->data)[i] = amp0 * amp * complex_exp(-I * phi);
    }
  }
...
int IMRPhenomDGenerateh22FDAmpPhase_internal(
  ...
  *h22 = CreateAmpPhaseFDWaveform(n);
  ...
  double fRef = (fRef_in == 0.0) ? pAmp->fmaxCalc/M_sec : fRef_in;
    ...
    //printf("%e %e %e\n", fRef, pAmp->fmaxCalc/M_sec, freq->data[n-1]);
 ...
 // #pragma omp parallel for
  for (size_t i = 0; i < n; i++)
  {
    ...
    if(findT == 1)
    {
        ...
        double df=1.0e-6/M_sec;
        if(f[0]-df < 0.0) df = 0.5*f[0];
        double v = (4.0*PI*df);
        for (size_t i = 0; i < n; i++)
        {
            double fp = f[i]+df;
            ...
            double Mfp = M_sec * fp; // geometric frequency
            double Mfm = M_sec * fm; // geometric frequency
            ...
            double phip = IMRPhenDPhase(Mfp, pPhi, pn, &powers_of_fp, &phi_prefactors);
            phip -= t0*(Mfp-MfRef) + phi_precalc;
            double phim = IMRPhenDPhase(Mfm, pPhi, pn, &powers_of_fm, &phi_prefactors);
...
//     return finspin;
// }
#endif // __IMRPHENOMD_INTERNALS_H__